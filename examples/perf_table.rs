...
fn main() {
    ...
    println!(
        "--- DBOBJ vs SQLite Performance Summary ({} rows) ---",
        row_count
    );
    ...
    // 1. Single Insert
    {
        ...
        conn.execute("CREATE TABLE users (val INTEGER)", [])
            .unwrap();
        let start = Instant::now();
        for i in 0..1000 {
            conn.execute(
                "INSERT INTO users (val) VALUES (?1)",
                sqlite_params![i as i64],
            )
            .unwrap();
        }
    ...
    // 2. Batch Insert
    {
        ...
        for _ in 0..100 {
            ...
            {
                let mut stmt = tx
                    .prepare_cached("INSERT INTO users (val) VALUES (?)")
                    .unwrap();
                for i in 0..100 {
    ...
    // 3. Read by ID
    {
        ...
        let conn = Connection::open_in_memory().unwrap();
        conn.execute(
            "CREATE TABLE users (id INTEGER PRIMARY KEY, val INTEGER)",
            [],
        )
        .unwrap();
        for i in 0..1000 {
            conn.execute(
                "INSERT INTO users (id, val) VALUES (?1, ?2)",
                sqlite_params![i as i64, i as i64],
            )
            .unwrap();
        }
        let start = Instant::now();
        for i in 0..10000 {
            let _: i64 = conn
                .query_row(
                    "SELECT val FROM users WHERE id = ?",
                    sqlite_params![(i % 1000) as i64],
                    |r| r.get(0),
                )
                .unwrap();
        }
    ...
    // 4. Indexed Search
    {
        ...
        let batch: Vec<Vec<Value>> = (0..10000)
            .map(|i| vec![Value::from(format!("user{}", i))])
            .collect();
        db.insert_batch_values("users", batch).unwrap();
        ...
        for _ in 0..1000 {
            let _ = db
                .find("users", "username", Value::from("user5000"))
                .unwrap();
        }
        let db_time = start.elapsed() / 1000;
        ...
        {
            let mut stmt = tx
                .prepare_cached("INSERT INTO users (username) VALUES (?)")
                .unwrap();
            for i in 0..10000 {
        ...
        for _ in 0..1000 {
            let _: String = conn
                .query_row(
                    "SELECT username FROM users WHERE username = 'user5000'",
                    [],
                    |r| r.get(0),
                )
                .unwrap();
    ...
    // 5. Hash Join
    {
        ...
        let u_batch: Vec<Vec<Value>> = (0..row_count)
            .map(|i| vec![Value::from(i as i64)])
            .collect();
        let p_batch: Vec<Vec<Value>> = (0..row_count)
            .map(|i| vec![Value::from(i as i64)])
            .collect();
        ...
        let start = Instant::now();
        let mut stmt = conn
            .prepare("SELECT u.id, p.uid FROM u JOIN p ON u.id = p.uid")
            .unwrap();
