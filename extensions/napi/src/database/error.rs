...
impl From<&str> for DbError {
    fn from(msg: &str) -> Self {
        let code = if msg.contains("not found") {
            "TABLE_NOT_FOUND".into()
        } else if msg.contains("Schema violation") || msg.contains("schema") {
            "SCHEMA_VIOLATION".into()
        } else if msg.contains("already exists") || msg.contains("Duplicate") {
            "DUPLICATE_KEY".into()
        } else if msg.contains("not nullable") || msg.contains("nullable") {
            "NULLABLE_VIOLATION".into()
        } else {
            "UNKNOWN".into()
        };
        DbError {
...
pub(crate) fn map_err<T>(result: Result<T, impl ToString>) -> napi::Result<T> {
    result.map_err(|e| {
        ...
        let code = if msg.contains("not found") {
            "TABLE_NOT_FOUND"
        } else if msg.contains("Schema violation") {
            "SCHEMA_VIOLATION"
